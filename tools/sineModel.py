...
def sineTracking(pfreq, pmag, pphase, tfreq, freqDevOffset=20, freqDevSlope=0.01, minFreq=200, maxFreq=500):
	"""
	...
	if incomingTracks.size > 0:                                 # if incoming tracks exist
		for i in magOrder:                                        # iterate over current peaks
			...
			if freqDistance < (freqDevOffset + freqDevSlope * pfreq[i]) and pfreq[i] > minFreq and pfreq[i] < maxFreq:  # choose track if distance is small
...
def sineModelAnal(x, fs, w, N, H, t, maxnSines = 100, minSineDur=.01, freqDevOffset=20, freqDevSlope=0.01, minFreq=200, maxFreq=950):
	"""
	...
	while pin<pend:                                         # while input sound pointer is within sound
		...
		tfreq, tmag, tphase = sineTracking(ipfreq, ipmag, ipphase, tfreq, freqDevOffset, freqDevSlope, minFreq, maxFreq)
